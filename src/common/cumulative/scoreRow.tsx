...
import ScoreDailyTrendReport from '../ScoreDailyTrendReport';
import ScoreMonthlyTrendReport from '../ScoreMonthlyTrendReport';
...
class ScoreRow extends React.Component<IProps, IState> { 
    ...
    generateMonthChartData = (diffMonthsAbs: any, monthsValue: any) => {
        try{
            ...
            BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `cumulative scoreRow generateMonthChartData catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
            return null;
        }
    }
    ...
    generateDateChartData = (diffDaysAbs: any, datesValue: any) => {
        try{
            ...
            toastr.error('Something went wrong!, please refresh the page or try again later.');
            BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `cumulative scoreRow generateDateChartData catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
            return null;
        }
    }
    ...
    tabsCallback(key: any) {
        try{
            ...
            toastr.error('Something went wrong!, please refresh the page or try again later.');
            BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `cumulative scoreRow tabsCallback catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
        }
    }
        ...
        return (
            ...
            <div className="sm-question-view clearfix sm-corner-a " sm-question-id="414992057" view-role="summaryBenchmarkQuestionViewContainer">
                ...
                <div sm-questionview-header="" className="sm-question-view-header clearfix" style={{ marginBottom: '20px' }}>
                    <span className="sm-question-number txt-shadow-lt">Q{this.props.question.no} Net Promoter Score {this.props.question.departmentMatchedWithFilterName ? `(KPI for ${this.props.question.departmentMatchedWithFilterName} Department)`: ''} {this.props.question.areaOfImpactMatchedWithFilterName ? `(Area of impact for ${this.props.question.areaOfImpactMatchedWithFilterName})`: ''}</span>
                    <div className="sm-question-btns clearfix">
                        {/* <span className="sm-float-r"><a href="#" className="wds-button wds-button--util-light wds-button--sm action-menu" onClick={(e) => this.props.exportHandler(this.props.question.no)}>Export</a></span> */}
                    </div>
                </div>
                ...
                <div className="sm-questionview-content">
                    ...
                    <div className="question-heading-ctnr">
                        <h1 question-heading="" className="sm-questiontitle" title={this.props.question.label}>{this.props.question.label}</h1>
                    </div>
                                ...
                                <div className="daily-trend-report">
                                    <div className="daily-trend-report-container">
                                        <div className="sm-chart">
                                            <ScoreDailyTrendReport height={350} data={this.state.dateChartData} padding={['20', '80', '120', '60']} rotate={30} disable={ false }/>
                                        </div>
                                    </div>
                                </div>
                            </TabPane>
                            <TabPane tab="Monthly Trend Report" key="monthly">
                                <div className="monthly-trend-report">
                                    <div className="monthly-trend-report-container">
                                        <div className="sm-chart">
                                            <ScoreMonthlyTrendReport height={350} data={this.state.monthChartData} padding={['20', '40', '100', '60']} rotate={30} disable={ false }/>
                                        </div>
                                    </div>
                                </div>
                            </TabPane>
