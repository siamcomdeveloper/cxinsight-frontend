...
import CheckboxDailyTrendReport from '../CheckboxDailyTrendReport';
import CheckboxMonthlyTrendReport from '../CheckboxMonthlyTrendReport';
...
class CheckboxRow extends React.Component<IProps, IState> { 
    ...
    generateMonthChartData = (diffMonthsAbs: any, monthsValue: any) => {
        try{
            ...
            BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `cumulative checkboxRow generateMonthChartData catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
            return null;
        }
    }
    ...
    generateDateChartData = (diffDaysAbs: any, datesValue: any) => {
        try{
            ...
            toastr.error('Something went wrong!, please refresh the page or try again later.');
            BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `cumulative checkboxRow generateDateChartData catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
            return null;
        }
    }
    ...
    tabsCallback(key: any) {
        try{
            ...
            toastr.error('Something went wrong!, please refresh the page or try again later.');
            BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `cumulative checkboxRow tabsCallback catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
        }
    }
                            ...
                            <div className="daily-trend-report">
                                <div className="daily-trend-report-container">
                                    <div className="sm-chart">
                                        <CheckboxDailyTrendReport height={chartHeight} data={this.state.dateChartData} padding={['20', '80', paddingBottomPercent+20, '60']} rotate={30} disable={ false }/>
                                    </div>
                                </div>
                            </div>
                        </TabPane>
                        <TabPane tab="Monthly Trend Report" key="monthly">
                            <div className="monthly-trend-report">
                                <div className="monthly-trend-report-container">
                                    <div className="sm-chart">
                                        <CheckboxMonthlyTrendReport height={chartHeight} data={this.state.monthChartData} padding={['20', '40', paddingBottomPercent, '60']} rotate={30} disable={ false }/>
                                    </div>
                                </div>
                            </div>
                        </TabPane>
