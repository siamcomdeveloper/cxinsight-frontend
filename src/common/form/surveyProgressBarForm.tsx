/* eslint-disable import/first */
import * as React from "react";
import { Form, Input, Button, Spin, Radio, Switch } from 'antd';
import { FormComponentProps } from "antd/lib/form";
import { RadioChangeEvent } from 'antd/lib/radio';
import BaseService from '../../service/base.service';
import { getJwtToken } from '../../helper/jwt.helper';
import * as toastr from 'toastr';
import Surveys from "../../models/surveys";
import { SketchPicker } from 'react-color';
import reactCSS from 'reactcss'
import { History } from 'history';

const formItemLayout = {
    labelCol: { span: 24 },
    wrapperCol: { span: 24 },
};

interface Props extends FormComponentProps{
  survey: Surveys;
  history: History;
    match:{ 
        isExact: boolean
        params: {
            xSite: string
        },
        path: string,
        url: string,
    }
}
interface IState{
  isLoading: boolean,
  survey: Surveys;
  fontColor: any;
  displayColorPickerBar: any;
  displayColorPickerText: any;
}

class EditForm extends React.Component<Props, IState, any> {

    constructor(props: Props & FormComponentProps) {
      super(props);

      // console.log(this.props.survey);

      this.state = {
        isLoading: true,
        survey: this.props.survey,
        fontColor: [],
        displayColorPickerBar: false,
        displayColorPickerText: false,
      }
    }

    componentDidMount() { 

      // console.log('CreateForm componentDidMount');

      const jwt = getJwtToken();
        
      BaseService.getJSON(this.props.match.params.xSite, "/color", '', jwt).then(
          (rp) => {
              try{
                  if (rp.Status) {
                      // const colors = rp.Data.result.recordset;
                      const colors = rp.Data.result.recordset.map((colorData: any) => {return colorData.hex_code;});
                      // console.log('colors', colors);
                      
                      this.setState({
                          fontColor: colors
                      }, () => {
                          this.setState({ isLoading: false }, () =>{ 
                            this.props.form.setFieldsValue({
                              enable_progressbar: this.props.survey.enable_progressbar,
                              progressbar_type: this.props.survey.progressbar_type,
                              progressbar_background_color: this.props.survey.progressbar_background_color,
                              progressbar_text_color: this.props.survey.progressbar_text_color,
                            }, () => { /*console.log('after')*/ });
                          });
                        });
                        
                      } else {
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: 'surveyRenameForm componentDidMount BaseService.getJSON /color else', message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                }catch(error){ 
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: 'surveyRenameForm componentDidMount BaseService.getJSON /color catch', message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );

    }
    
    selectUpdate = (obj: any, selectKeys: any, params: any) => {
        const clone = Object.assign({}, obj);

        // console.log(selectKeys);

        for (const key in obj) {
            let matched = false;

            selectKeys.forEach((selectKey: any, index: any) => {
                // console.log('index', index);
                if(selectKey === key){
                    matched = true;
                    clone[selectKey] = params[index];
                }
            });
            if(!matched) {
                delete clone[key];
                // console.log('delete!');
            }
        }

        return clone;
    }
  
    check = () => {
      this.props.form.validateFields(err => {
        if (!err) {
          // console.log('check', this.state.survey);
  
          let fields = [] as any;
              
          fields = ['enable_progressbar', 'progressbar_type', 'progressbar_background_color', 'progressbar_text_color'];

          const jwt = getJwtToken();
          BaseService.update(this.props.match.params.xSite, "/surveys/", this.state.survey.id, this.selectUpdate(this.state.survey, fields, [ this.state.survey.enable_progressbar, this.state.survey.progressbar_type, this.state.survey.progressbar_background_color, this.state.survey.progressbar_text_color ]), jwt).then(
              (rp) => {
                  try{
                      if (rp.Status) {
                          // console.log(rp);
                          toastr.success(rp.Messages);
                          setTimeout(function(){ window.location.reload(); }, 500);
                      } else {
                          // toastr.error(rp.Messages);
                          toastr.error('Something went wrong!, please refresh the page or try again later.');
                          BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `surveySubmitButtonForm check BaseService.update /surveys/${this.state.survey.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                      } 
                  }catch(error){ 
                      toastr.error('Something went wrong!, please refresh the page or try again later.');
                      BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `surveySubmitButtonForm check BaseService.update /surveys/${this.state.survey.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                  }
              }
          );

        }
      });
    };
  
    onChange = (e: React.ChangeEvent<HTMLInputElement>)=> {
      this.onFieldValueChange(e.target.id, e.target.value)
    }

    onRichChange = (id: any, richTextEditor: any) => {

      // console.log(`onRichChange id`, id);
      // console.log(`onRichChange id_html`, id+'_html');
      // console.log(`onRichChange richTextEditor`, richTextEditor);
      this.onFieldValueChange(id, richTextEditor.html.trim()); 
        this.props.form.setFieldsValue({
            [id]: richTextEditor.length === 1 ? '' :richTextEditor.html.trim()
        });
  };

    onFieldValueChange = (fieldName: string, value: any) => { 
      const nextState = {
          ...this.state,
          survey: {
              ...this.state.survey,
              [fieldName]: value,
          }
      };

      this.setState(nextState);
    }

    handleClickBar = () => {
      this.setState({ displayColorPickerBar: !this.state.displayColorPickerBar })
    };

    handleCloseBar = () => {
        this.setState({ displayColorPickerBar: false })
    };

    handleClickText = () => {
      this.setState({ displayColorPickerText: !this.state.displayColorPickerText })
    };

    handleCloseText = () => {
        this.setState({ displayColorPickerText: false })
    };

    handleChangeBar = (color: any) => {
        this.setState({ 
          survey: { 
            ...this.state.survey,
            [`progressbar_background_color`]: color.hex 
          }
        })
    };

    handleChangeText = (color: any) => {
        this.setState({ 
          survey: { 
            ...this.state.survey,
            [`progressbar_text_color`]: color.hex 
          }
        })
    };

    onChangeProgress = (value: any) => {
      this.setState({
        survey: { 
          ...this.state.survey,
          [`enable_progressbar`]: value ? 1 : 0
        }
      });
    };

    onChangeBarType = (e: RadioChangeEvent) => {
      this.setState({
        survey: { 
          ...this.state.survey,
          [`progressbar_type`]: e.target.value
        }
      });
    };

    render() {
      const { getFieldDecorator } = this.props.form;

      const styles = reactCSS({
        'default': {
          colorBar: {
            width: '36px',
            height: '14px',
            borderRadius: '2px',
            backgroundColor: this.state.survey.progressbar_background_color
          },
          colorText: {
            height: '16px',
            borderRadius: '2px',
            padding: '8px',
            backgroundColor: this.state.survey.progressbar_background_color,
            color: this.state.survey.progressbar_text_color
          },
          swatch: {
            padding: '5px',
            background: '#fff',
            borderRadius: '1px',
            boxShadow: '0 0 0 1px rgba(0,0,0,.1)',
            display: 'inline-block',
            cursor: 'pointer',
          },
        },
      });
      
      if (this.state.isLoading) {
        return <div id="overlay"> <Spin size="large" tip="Loading..."></Spin> </div>
      }
      
      return (
        <div className="design-question-modal">
          <Form.Item>
            <Switch onChange={(value) => this.onChangeProgress(value)} checked={this.state.survey.enable_progressbar ? true : false} style={{ marginRight: "10px" }}/>Enable Progress Bar
          </Form.Item>
          <Form.Item>
            <Radio.Group value={this.state.survey.progressbar_type ? 1 : 0} onChange={this.onChangeBarType}>
              <Radio value={0}>Scroll screen</Radio>
              <Radio value={1}>Bottom of page</Radio>
            </Radio.Group>
          </Form.Item>
          <Form.Item>
            <label className="sm-label" style={{ marginBottom: '0' }}>Bar Color Theme:</label>
            <div style={{ display: 'inline-block', marginLeft: '20px', lineHeight: 0, bottom: '-3px', position: 'absolute' }}>
              <div style={ styles.swatch } onClick={ this.handleClickBar }>
                <div style={ styles.colorBar } />
              </div>

              { this.state.displayColorPickerBar ? 
                <div style={{ position: 'absolute', zIndex: 2 }}>
                  <div style={{ position: 'fixed', top: '0px', right: '0px', bottom: '0px', left: '0px', }} onClick={ this.handleCloseBar }></div>
                  <SketchPicker color={ this.state.survey.progressbar_background_color ? this.state.survey.progressbar_background_color : '#fff' } onChange={ this.handleChangeBar } />
                </div> 
              : null }
            </div>
          </Form.Item>
          <Form.Item>
            <label className="sm-label" style={{ marginBottom: '0' }}>Text Color:</label>
            <div style={{ display: 'inline-block', marginLeft: '20px', lineHeight: 0, bottom: '-3px', position: 'absolute' }}>
              <div style={ styles.swatch } onClick={ this.handleClickText }>
                <div style={ styles.colorText } >{`Text`}</div>
              </div>

              { this.state.displayColorPickerText ? 
                <div style={{ position: 'absolute', zIndex: 2 }}>
                  <div style={{ position: 'fixed', top: '0px', right: '0px', bottom: '0px', left: '0px', }} onClick={ this.handleCloseText }></div>
                  <SketchPicker color={ this.state.survey.progressbar_text_color ? this.state.survey.progressbar_text_color : '#fff' } onChange={ this.handleChangeText } />
                </div> 
              : null }
            </div>
          </Form.Item>

          <footer className="wds-modal__foot">
            <div className="wds-modal__actions-right">
              <Button type="primary" className="wds-button wds-button--primary wds-button--solid wds-button--md" id="newSurvey" onClick={this.check}>
                APPLY
              </Button>
            </div>
          </footer>
        </div>
      );
    }
  }
  
const messageEditForm = Form.create<Props>()(EditForm);
export default messageEditForm;
