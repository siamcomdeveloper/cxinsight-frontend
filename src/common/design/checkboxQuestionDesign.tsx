...
class CheckboxQuestionDesign extends React.Component<IProps, IState> { 
    ...
    constructor(props: IProps) {
        ...
        // console.log('CheckboxQuestionDesign constructor', props);
    }

    public componentDidMount() { 
        // console.log('CheckboxQuestionDesign componentDidMount');
    ...
    public renderImage() { 
        try{
            // console.log('CheckboxQuestionDesign renderImage');
            ...
            BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `design checkboxQuestionDesign renderImage catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
        }
    }

    public renderElement() { 
        try{
            // console.log('CheckboxQuestionDesign renderElement');
            ...
            toastr.error('Something went wrong!, please refresh the page or try again later.');
            BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `design checkboxQuestionDesign renderElement catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
        }
    }
    ...
    getChoiceRow = (index: any, choice: any, choiceHtml: any, weight: any) => {
        ...
        return (<div key={index} className="answer-option-cell">
                    <div id={"question-"+this.props.question.no+"-checkbox-option-"+index} data-value={parseInt(weight)} className="checkbox-button-container">
                        <input name="414992059" type="checkbox" className="checkbox-button-input" value={weight}/>
                        <label className={ 'answer-label checkbox-button-label no-touch touch-sensitive clearfix '}>
    ...
    componentWillReceiveProps(props: any) {
        // console.log('CheckboxQuestionDesign componentWillReceiveProps', props);
        this.setState({ requiredLabelState: props.requiredLabelStatus });
    }

    shouldComponentUpdate(nextProps:any, nextState:any){
        // console.log('CheckboxQuestionDesign shouldComponentUpdate', nextProps, nextState);
        return true;
    }

    componentDidUpdate(prevProps:any, prevState:any){
        // console.log('CheckboxQuestionDesign componentDidUpdate', prevProps, prevState);
    }

    componentWillUnmount(){
        // console.log('CheckboxQuestionDesign componentWillUnmount');
    }

    componentWillMount(){
        // console.log('CheckboxQuestionDesign componentWillMount');
    }

    componentWillUpdate(){
        // console.log('CheckboxQuestionDesign componentWillUpdate');
        // this.renderElement();
    }
        ...
        return (
            <div className="question-row clearfix">
                <div className="question-container survey-submit-actions" style={{ cursor: 'default' }}>
                    ...
                    <Popconfirm
                        // title={` (this action will remove all Skip "Logic")`}
                    ...
                    <div id="question-field-414992058" className="question-checkbox-choice qn question vertical_two_col"> 
                        ...
                        {/* <h4 className={ this.props.skipLogicStatus ? "question-title-container" : "question-title-container hidden" }><span className="user-generated notranslate" style={{fontStyle: 'italic'}}>{this.props.skipLogicText}</span></h4> */}
                        <fieldset className={ "question-fieldset" }>
                            ...
                            <div className={ this.props.question.showConsentSection ? "other-answer-container" : 'other-answer-container hidden' }>
                                <br/>
                                <Button type="primary" className="buttons" style={{ width: '100%', background: 'rgb(0, 71, 178)' }}>Open Data Subject Signature Pad</Button>

                                <li key={`background-upload`} className="c1 dta cta acc_question_types ui-draggable customer-consent" style={{ cursor: 'default', textAlign: 'center' }}>
                                    <h3 style={{ minHeight: '25px', fontSize: '14px' }}>Upload A Customer Consent Photo</h3>
                                </li>
                            </div>
...
export default CheckboxQuestionDesign;