...
class NewQuestionTemplateForm extends React.Component<Props, IState, any> {
    ...
    // console.log('NewQuestionTemplateForm constructor', props);
    }

    componentDidMount() { 

      // console.log('NewQuestionTemplateForm componentDidMount');
        this.renderElement();
    }

    componentWillReceiveProps(props: any) {
      // console.log('NewQuestionTemplateForm componentWillReceiveProps', props);
    }
    ...
    onSaveNew(){
        ...
        if(this.state.newToPosOption){
            ...
            pathUrl = `${this.state.survey.id}/${this.state.question.typeId}/${this.state.question.templateId}/${this.state.newToPageOption}/${this.state.newToQuestionOption}/${toPosition}/${this.state.question.questionBankLang}`;
        }
        //check add a question to a new page
        else{
            pathUrl = `${this.state.survey.id}/${this.state.question.typeId}/${this.state.question.templateId}/${this.state.newToPageOption}/${this.state.survey.num_question}/${this.state.question.questionBankLang}`;
        }
      ...
      // console.log(`onSaveNew to Page: ${this.state.newToPageOption} Question: ${this.state.newToQuestionOption} Position: ${this.state.newToPosOption} toPosition: ${toPosition} pathUrl`, pathUrl, this.state.newToPosOption ? true : false);
        ...
        BaseService.update(this.props.match.params.xSite, "/question/design/newtemplate/", pathUrl, '', jwt).then(
            (rp) => {
                if (rp.Status) {
                  // console.log(rp);
                    ...
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `newQuestionTemplateForm onSaveNew BaseService.update /question/design/new/${pathUrl} catch`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                }
            }
        );
    }
    ...
    public async renderElement() { 
      try{
        // console.log('NewQuestionTemplateForm renderElement');
        ...
        // console.log('allQuestionOptionElementCopy removed', allQuestionOptionElementCopy);
          ...
          toastr.error('Something went wrong!, please refresh the page or try again later.');
          BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `newQuestionTemplateForm renderElement catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
        }
    }
                ...
                try{
                    if (rp.Status) {
                      ...
                      // console.log(`get question ${i} count = `, rp.Data.recordset.length);
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `newQuestionTemplateForm getPageNo BaseService.get<Question> /question/${surveyId}/${i} else rp.Status`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                        return false;
                    }
                }catch(error){
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `newQuestionTemplateForm getPageNo BaseService.get<Question> /question/${surveyId}/${i} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }//async (rp)
        );//call question api
    }
                ...
                try{
                    if (rp.Status) {
                      ...
                      // console.log(`get question ${no} count = `, rp.Data.recordset.length);
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `newQuestionTemplateForm getQuestionOptionRow BaseService.get<Question> /question/${this.state.survey.id}/${no} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                        return <div key={'new-option-'+no}></div>;
                    }
                  }catch(error){
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `newQuestionTemplateForm getQuestionOptionRow BaseService.get<Question> /question/${this.state.survey.id}/${no} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }//async (rp)
        );//call question api
              ...
              <div className="questionSetting">
                  <fieldset className="expander expand moveSetting">
                        ...
                        <label className="switch">
                            Add this question template and put it on ...
...
const newQuestionForm = Form.create<Props>()(NewQuestionTemplateForm);
