...
import MessageEditModal from '../../../common/modal/smsMessageEditModal';
...
export default class CollectorMessage extends React.Component<IProps, IState> {
                ...
                try{
                    if (rp.Status) {
                      ...
                      // console.log('componentDidMount', collector.survey_id);
                                ...
                                try{
                                    if (rp.Status) {
                                      // console.log('componentDidMount survey', rp.Data);
                                        ...
                                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage componentDidAmount BaseService.get<Surveys> /surveys/${rp.Data.recordset[0].survey_id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                                    }
                                }catch(error){ 
                                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage componentDidAmount BaseService.get<Surveys> /surveys/${rp.Data.recordset[0].survey_id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                                }
                            }
                        ...
                        (rp) => {
                            try{
                                if (rp.Status) {
                                    ...
                                    // toastr.error(rp.Messages);
                                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage componentDidMount BaseService.getAll<Project>(this.props.match.params.xSite, '/projects/') else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                                }
                            }catch(error){ 
                                toastr.error('Something went wrong!, please refresh the page or try again later.');
                                BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage componentDidMount BaseService.getAll<Project>(this.props.match.params.xSite, '/projects/') catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                            }
                        });// get projects
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage componentDidAmount BaseService.get<Collector> /collector/${this.props.match.params.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                }catch(error){ 
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage componentDidAmount BaseService.get<Collector> /collector/${this.props.match.params.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        ...
        BaseService.getJSON(this.props.match.params.xSite, "/sms/list/", this.props.match.params.id, jwt).then(
            (rp) => {
                try{
                    if (rp.Status) {
                      ...
                      // console.log('noResponsesPercent', noResponsesPercent);
                        ...
                        this.setState({
                            ...
                            BaseService.getJSON(this.props.match.params.xSite, "/sms/messagehistory/list/", this.props.match.params.id, jwt).then(
                                (rp) => {
                                    try{
                                        if (rp.Status) {
                                          ...
                                          // console.log('getJSON messagehistory', rp.Data.result.recordset.length);
                                            ...
                                            BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage fetch BaseService.getJSON /sms/messagehistory/list/${this.props.match.params.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                                            ReactDOM.render(<Empty style={{ padding: '50px 0' }} description={<span> No messages </span>}/>, document.getElementById('message-history-list'));
                                            this.setState({ loadingMessageHistory: false });
                                        }
                                    } catch(error){
                                        ...
                                        this.setState({ loadingMessageHistory: false });
                                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage fetch BaseService.getJSON /sms/messagehistory/list/${this.props.match.params.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                                    }
                                }
                            );
                    
                            BaseService.getJSON(this.props.match.params.xSite, "/sms/messagefollowup/list/", this.props.match.params.id, jwt).then(
                                (rp) => {
                                    try{
                                        if (rp.Status) {
                                          ...
                                          // console.log('getJSON messagefollowup', rp.Data.result.recordset.length);
                                            ...
                                            ReactDOM.render(<Empty style={{ padding: '50px 0' }} description={<span> No messages </span>}/>, document.getElementById('message-followup-list'));
                                            this.setState({ loadingMessageFollowUp: false });
                                            BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage fetch BaseService.getJSON /sms/messagefollowup/list/${this.props.match.params.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                                        }
                                    } catch(error){
                                        // ReactDOM.render(<Empty style={{ padding: '50px 0' }} description={<span> No messages </span>}/>, document.getElementById('message-followup-list'));
                                        this.setState({ loadingMessageFollowUp: false });
                                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage fetch BaseService.getJSON /sms/messagefollowup/list/${this.props.match.params.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                                    }
                                }
                            );
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage fetch BaseService.getJSON /sms/list/${this.props.match.params.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                        this.setState({ loading: false });
                    }
                } catch(error){
                    ...
                    this.setState({ loading: false });
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage fetch BaseService.getJSON /sms/list/${this.props.match.params.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );
    };
    ...
    onSave = () => { 
      ...
      // console.log(`onSave`, this.state.collector.id);
        ...
        this.state.dataImport.map( (object: any, i: any) => {
            ...
            // console.log(object);
            BaseService.create<SMS>(this.props.match.params.xSite, "/sms/", this.selectUpdate(object, ['mobile_number', 'first_name', 'last_name', 'collector_id', 'customer_id', 'custom_group'], [object.mobile_number, object.first_name, object.last_name, object.collector_id, object.customer_id, object.custom_group]), jwt).then(
                (rp) => {
                    try{
                        if (rp.Status) {
                            ...
                            // toastr.error(rp.Messages);
                            toastr.error('Something went wrong!, please refresh the page or try again later.');
                            BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onSave BaseService.create<Email> /sms/ catch`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                        }
                    }catch(error){ 
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onSave BaseService.create<Email> /sms/ catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                    }
                }
            );

            // console.log(this.selectUpdate(object, ['mobile_number', 'first_name', 'last_name', 'collector_id'], [object.mobile_number, object.first_name, object.last_name, object.collector_id]));
        });

        // console.log('Import Completed!');
        // toastr.success('Import Completed!'); 

        // BaseService.create<Email>("/sms/", this.selectUpdate(this.state.dataImport, ['cutoff_date'], [datetime])).then(
        //     (rp) => {
        //         if (rp.Status) {
        //             toastr.success('Collector created.'); 
        //             // this.history.push('/collectors/'+this.state.survey.id);
                ...
                try{
                    if (rp.Status) {
                      ...
                      // console.log('Create showCollectorModal', this.state);
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage showCollectorModal BaseService.get<Surveys> /collector/typeName/${typeId} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                }catch(error){ 
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage showCollectorModal BaseService.get<Surveys> /collector/typeName/${typeId} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );
    ...
    menuSendReminder = () => (
        <Menu>
            {/* <Menu.Item key="close">
                <a  href="# " onClick={()=>this.updateStatus(4, 'CLOSED')} style={{ textDecoration: 'none' }}>Automate a reminder sms</a>
            </Menu.Item> */}
            <SubMenu key="sub3" title="Send a one-off reminder sms">
                <Menu.Item key="7"><a  href="# " onClick={()=>this.followUpSendSmses(1)} style={{ textDecoration: 'none' }}>No response</a></Menu.Item>
                <Menu.Item key="8"><a  href="# " onClick={()=>this.followUpSendSmses(2)} style={{ textDecoration: 'none' }}>Partial response</a></Menu.Item>
                <Menu.Item key="9"><a  href="# " onClick={()=>this.followUpSendSmses(3)} style={{ textDecoration: 'none' }}>Both</a></Menu.Item>
            </SubMenu>
            {/* <Menu.Item key="open">
                <a  href="# " onClick={()=>this.updateStatus(2, 'OPEN')} style={{ textDecoration: 'none' }}>Send a one-off reminder sms</a>
            </Menu.Item> */}
        </Menu>
    );

    followUpSendSmses = (functionId: any) => {
    
      // console.log('followUpSendSmses functionId', functionId);

        const jwt = getJwtToken();
        BaseService.getJSON(this.props.match.params.xSite, "/sms/followup/list/", this.props.match.params.id + `/${functionId}`, jwt).then(
            (rp) => {
                try{
                    if (rp.Status) {
                      // console.log('rp', rp);
                                ...
                                try{
                                    if (rp.Status) {
                                      ...
                                      // console.log('getJSON messagefollowup', rp.Data.result.recordset.length);
                                        ...
                                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage followUpSendSmses BaseService.getJSON /sms/messagefollowup/list/${this.props.match.params.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                                        ReactDOM.render(<Empty style={{ padding: '50px 0' }} description={<span> No messages </span>}/>, document.getElementById('message-followup-list'));
                                        this.setState({ loadingMessageFollowUp: false });
                                    }
                                } catch(error){
                                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage followUpSendSmses BaseService.getJSON /sms/messagefollowup/list/${this.props.match.params.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                                    // ReactDOM.render(<Empty style={{ padding: '50px 0' }} description={<span> No messages </span>}/>, document.getElementById('message-followup-list'));
                                    this.setState({ loadingMessageFollowUp: false });
                                }
                            }
                        ...
                        // toastr.error(rp.Messages + ' - something went wrong!');
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage followUpSendSmses BaseService.getJSON /sms/followup/list/${this.props.match.params.id}/${functionId} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                } catch(error){
                    // toastr.error(rp.Messages + ' - something went wrong!');
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage followUpSendSmses BaseService.getJSON /sms/followup/list/${this.props.match.params.id}/${functionId} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );
    }
            ...
            (rp) => {
                try{
                    if (rp.Status) {
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage updateStatus BaseService.update /collector/${this.state.collector.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                }catch(error){ 
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage updateStatus BaseService.update /collector/${this.state.collector.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );
        //DELETE
                ...
                try{
                    if (rp.Status) {
                      ...
                      // console.log('onChangeSend datetime', datetime);
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onChangeSend BaseService.update /collector/${this.state.collector.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                }catch(error){ 
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onChangeSend BaseService.update /collector/${this.state.collector.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );
    }
                ...
                try{
                    if (rp.Status) {
                      ...
                      // console.log('onChangeCutoff datetime', datetime);
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onChangeCutoff BaseService.update /collector/${this.state.collector.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                }catch(error){ 
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onChangeCutoff BaseService.update /collector/${this.state.collector.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );
    }
                ...
                try{
                    if (rp.Status) {
                      ...
                      // console.log('onCutoffOk cutoffDatetime', cutoffDatetime);
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onCutoffOk BaseService.update /collector/${this.state.collector.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                }catch(error){ 
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onCutoffOk BaseService.update /collector/${this.state.collector.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );
                ...
                try{
                    if (rp.Status) {
                      ...
                      // console.log('onSendOk sendDatetime', sendDatetime);
                        ...
                        BaseService.getJSON(this.props.match.params.xSite, "/sms/schedulesendsmses/list/", this.props.match.params.id, jwt).then(
                            (rp) => {
                                try{
                                    if (rp.Status) {
                                      // console.log('rp', rp);
                                        ...
                                        // toastr.error(rp.Messages + ' - something went wrong!');
                                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onSendOk BaseService.getJSON /sms/schedulesendsmss/list/${this.props.match.params.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                                    }
                                } catch(error){
                                    // toastr.error(rp.Messages + ' - something went wrong!');
                                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onSendOk BaseService.getJSON /sms/schedulesendsmss/list/${this.props.match.params.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                                }
                            }
                        );
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onSendOk BaseService.update /collector/${this.state.collector.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                }catch(error){ 
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onSendOk BaseService.update /collector/${this.state.collector.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );
            ...
            (rp) => {
                try{
                    if (rp.Status) {
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onRadioSendChange BaseService.update /collector/${this.state.collector.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                }catch(error){
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onRadioSendChange BaseService.update /collector/${this.state.collector.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );
    };
            ...
            (rp) => {
                try{
                    if (rp.Status) {
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onRadioCutoffChange BaseService.update /collector/${this.state.collector.id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                }catch(error){ 
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage onRadioCutoffChange BaseService.update /collector/${this.state.collector.id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );
    };
    ...
    handleInvite = (id: any) => {
      // console.log(`handleInvite id ${id} this.state.collector.type ${this.state.collector.type}`);
        ...
        BaseService.post(this.props.match.params.xSite, "/sms/send/invite/", obj, jwt).then(
            (rp) => {
                try{
                    if (rp.Status) {
                        ...
                        this.setState({ isLoadingSubmit: false });
                        // toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage handleInvite BaseService.post /sms/send/invite/ else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                }catch(error){ 
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage handleInvite BaseService.post /sms/send/invite/ catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );
    };
    ...
    handleDelete = (id: any) => {
      // console.log(`handleDelete id ${id} this.state.collector.type ${this.state.collector.type}`);
        ...
        const jwt = getJwtToken();
        BaseService.delete(this.props.match.params.xSite, '/sms/', id, jwt).then(
            (rp) => {
                try{
                    if (rp.Status) {
                        toastr.success(rp.Messages);    
                        ...
                        // toastr.error(rp.Messages);
                        toastr.error('Something went wrong!, please refresh the page or try again later.');
                        BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage handleDelete BaseService.delete /sms/${id} else`, message: `Messages: ${rp.Messages} | Exception: ${rp.Exception}` }, getJwtToken()).then( (rp) => { console.log(`Messages: ${rp.Messages} | Exception: ${rp.Exception}`); });
                    }
                }catch(error){ 
                    toastr.error('Something went wrong!, please refresh the page or try again later.');
                    BaseService.post(this.props.match.params.xSite, "/frontendlog/", { method: `collect sms manage handleDelete BaseService.delete /sms/${id} catch`, message: `catch: ${error}` }, getJwtToken()).then( (rp) => { console.log(`catch: ${error}`); });
                }
            }
        );
    };
        ...
        const columns: ColumnProps<never>[] = [
            {
                ...
                // sorter: true,
                sorter: (a: { mobile_number: string }, b: { mobile_number: string }) => a.mobile_number.length - b.mobile_number.length,
                width: '45%',
            },
            {
                title: 'FIRST NAME',
                                ...
                                <section className="weblink">
                                    <div id="edit-weblink" style={{ margin: '0', border: '0', padding: '0' }}>
                                        <div id="collector-status" className="clearfix">
                                            <div>
                                                Project / Branch : <b>{this.state.listProjects.map((project: any) => { if(project.id === this.state.collector.project_id) return project.name; })} <span id="edit-name-icon" onClick={()=>this.showCollectorModal(this.state.collector.type)} className="smf-icon notranslate">W</span></b>
                                            </div>
                                        ...
                                        <div className="accordion options">
                                            <div className="key open" style={{ maxHeight: '100%', borderTop: '1px solid lightgray' }}>
                                                ...
                                                <section id="panel-all-recipients" style={{ height: 'auto' }}>
                                                    ...
                                                    <div style={{ width: '100%' }}>
                                                        <Button type="primary" className='add-button' style={{ marginBottom: 15 }} onClick={()=>this.followUpSendSmses(4)}>
                                        ...
                                        <div className="accordion options">
                                            <div className="key open" style={{ maxHeight: '100%', borderTop: '1px solid lightgray' }}>
                                                ...
                                                <section id="panel-compose-message" style={{height: 'auto', paddingBottom: '40px'   }}>
                                                    ...
                                                    <article id="compose-sms" className="reading template" onClick={this.showMessageModal}>
                                                        ...
                                                        <div id="sms-preview" style={{ width: "100%" }}>
